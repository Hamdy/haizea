...
class OpenNebulaXMLRPCClient(object):
    ...
    def __init__(self, host, port, user, password):
        uri = "http://%s:%s@%s:%i" % (user, password, host, port)
    ...
    def hostpool_info(self):
        try:
            (rc, value, number) = self.rpc.one.hostpool.info(self.auth)
            if rc == False:
    ...
    def vmpool_info(self):
        try:
            (rc, value, err) = self.rpc.one.vmpool.info(self.auth, -2, -1, -1, 1) # -2: Get all VMs
            if rc == False:
                raise Exception("ONE reported an error: %s" % value)
            else:
